...
def _salary_text(
    ...
    if isinstance(
        ...
        minimum = salary_value.get(
            "minValue"
        )

        maximum = salary_value.get(
            "maxValue"
        )
...
def _parse_generic(
    ...
    *,
    ...
    if not title:
        raise ValueError(
            "could not identify a job title"
        )

    if not company:
        raise ValueError(
            "could not identify the hiring company"
        )

    if not description:
        raise ValueError(
            "could not identify a job description"
        )
...
def parse_job_page(
    ...
    *,
    ...
    Discovery metadata may provide a company name or external job ID when the
    public vacancy page itself omits those fields.
    """
    ...
    if (
        ...
        raise ValueError(
            "cannot parse page with "
            f"fetch status "
            f"{page.fetch_status!r}"
        )

    if not page.html:
        raise ValueError(
            "cannot parse a page without HTML"
        )

    page_url = str(
        page.final_url
        or page.requested_url
    )
    ...
    json_ld_job = extract_json_ld_job(
        extractor.json_ld_blocks
    )
